...
@app.get("/source/{filename}/content")
async def get_source_content(filename: str):
    """Get the text content of a specific document source.
    
    Args:
        filename: Name of the source (document) to view
    """
    try:
        content = vector_store.get_source_content(filename)
        return {"filename": filename, "content": content}
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving content for {filename}: {str(e)}"
        )


@app.delete("/source/{filename}")
async def delete_source(filename: str):
    """Delete a specific document source and its embeddings.
    
    Args:
        filename: Name of the source (document) to delete
    """
    try:
        success = vector_store.delete_source(filename)
        if success:
            return {"status": "success", "message": f"Source {filename} deleted successfully"}
        else:
            raise HTTPException(
                status_code=500,
                detail=f"Failed to delete source {filename}"
            )
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Error deleting source {filename}: {str(e)}"
        )
