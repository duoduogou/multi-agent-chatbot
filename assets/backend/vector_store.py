...
class VectorStore:
    ...
    def get_source_content(self, source_name: str) -> str:
        """
        Retrieve all document text content for a specific source.
        
        Args:
            source_name: The name of the source (usually filename)
            
        Returns:
            str: The concatenated text content of the source
        """
        try:
            # Retrieve documents for this source with a very high k to get everything
            # In a real-world scenario with massive files, we might need a more efficient way or pagination
            docs = self.get_documents(query="", k=1000, sources=[source_name])
            
            if not docs:
                return "No content found for this source."
                
            # Sort by whatever metadata we have that might indicate order if available,
            # otherwise just join. LangChain/Milvus doesn't guarantee order here.
            content = "\n\n".join([doc.page_content for doc in docs])
            return content
        except Exception as e:
            logger.error({
                "message": "Error getting source content",
                "source": source_name,
                "error": str(e)
            }, exc_info=True)
    ...
    def delete_source(self, source_name: str) -> bool:
        """
        Delete all document chunks associated with a source from Milvus.
        ...
        try:
            ...
            # First, let's try to find the file path to delete it from disk
            # We'll retrieve one doc to get the metadata
            docs = self.get_documents(query="", k=1, sources=[source_name])
            file_path = None
            if docs:
                file_path = docs[0].metadata.get("file_path")

            # Execute deletion in Milvus
            collection.delete(expr)
            self.flush_store()
            
            # Delete from disk if path was found
            if file_path and os.path.exists(file_path):
                try:
                    ...
                    logger.warning(f"Failed to delete file from disk: {disk_err}")
