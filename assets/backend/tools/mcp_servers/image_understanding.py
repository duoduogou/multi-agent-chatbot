...
model_client = OpenAI(
    base_url=f"http://qwen-vl-2b:8000/v1",
    api_key="api_key"
)
...
def explain_image(query: str, image: str):
    ...
    if image.startswith("http://") or image.startswith("https://"):
        try:
            print(f"Fetching image from URL: {image}")
            resp = requests.get(image, timeout=10)
            resp.raise_for_status()
            content_type = resp.headers.get("Content-Type", "image/jpeg")
            filetype = content_type.split("/")[-1] if "/" in content_type else "jpeg"
            b64_data = base64.b64encode(resp.content).decode("utf-8")
            print(f"Successfully fetched and encoded image from URL.")
        except Exception as e:
            print(f"Failed to fetch image from URL: {e}")
            raise ValueError(f"Could not fetch image from URL: {image}. Error: {e}")
            
        image_url_content = {
            "type": "image_url",
            "image_url": {
                "url": f"data:image/{filetype};base64,{b64_data}"
            }
        }
    else:
