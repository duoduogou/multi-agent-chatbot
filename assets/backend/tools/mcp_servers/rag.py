...
async def fetch_web_content(url: str) -> str:
    """Fetch and read the textual content of a public website URL.
    
    Use this tool when the user provides a URL and asks to summarize, analyze, or read its content.
    
    Args:
        url: The full HTTP/HTTPS URL to fetch.
        
    Returns:
        The text content of the website or an error message.
    """
    import requests
    from bs4 import BeautifulSoup
    
    logger.info({"message": "Fetching web content", "url": url})
    try:
        ...
        # Limit content length to avoid context overflow (e.g., first 5000 chars)
        max_chars = 8000
        if len(text) > max_chars:
            text = text[:max_chars] + "\n\n... (content truncated for length)"
            
        logger.info({"message": "Successfully fetched web content", "length": len(text)})
        return text
    except Exception as e:
        logger.error({"message": "Failed to fetch web content", "url": url, "error": str(e)})
