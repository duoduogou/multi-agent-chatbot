...
class ChatAgent:
    ...
    async def init_tools(self) -> None:
        ...
        # Add local tools
        await self.init_local_tools()

    async def init_local_tools(self) -> None:
        """Initialize local tools and add them to the agent."""
        local_tools = [
            StructuredTool.from_function(
                func=self.search_knowledge_base,
                name="search_knowledge_base",
                description="Search the knowledge base for relevant documents and information. Use this whenever the user asks a question that might be answered by the uploaded documents."
            )
        ]

        # Convert local tools to OpenAI format
        local_tools_openai = [convert_to_openai_tool(tool) for tool in local_tools]
        
        # Add to tools_by_name
        for tool in local_tools:
            self.tools_by_name[tool.name] = tool
            
        # Add to openai_tools
        for tool in local_tools_openai:
            self.openai_tools.append({"type": "function", "function": tool['function']})
            
        logger.debug(f"Added {len(local_tools)} local tools. Total tools: {len(self.tools_by_name)}")

    def search_knowledge_base(self, query: str, sources: list[str] = None) -> str:
        """Search the knowledge base for relevant documents.
        
        Args:
            query: The search query string
            sources: Optional list of specific source filenames to search within
            
        Returns:
            Formatted string of retrieved documents
        """
        try:
            # If no sources provided by the model, use the globally selected sources from config
            if not sources and hasattr(self, 'config_manager'):
                try:
                    config = self.config_manager.read_config()
                    if hasattr(config, 'selected_sources') and config.selected_sources:
                        sources = config.selected_sources
                        logger.info(f"Using selected sources from config: {sources}")
                except Exception as config_err:
                    logger.warning(f"Could not read selected sources from config: {config_err}")

            logger.info(f"Executing tool search_knowledge_base with query: {query}, sources: {sources}")
            # Use k=4 with truncation to avoid overwhelming the 1.5B model (PROVEN WORKING CONFIG)
            docs = self.vector_store.get_documents(query, k=4, sources=sources)
            
            # STRICT MODE: Only search selected sources. No fallback.
            # User requirement: "确保只有勾选中才使用检索" (Ensure only selected [files] are used)

            logger.info(f"Retrieved {len(docs)} documents for query: {query}")
            
            if not docs:
                return "No relevant documents found in the selected sources."
            
            # Simplified format for better stability
            result = f"Here is relevant information from the knowledge base for query '{query}'. Use this to answer the user's question:\n\n"
            for i, doc in enumerate(docs):
                source = doc.metadata.get("source", "Unknown")
                # Truncate to 600 chars to avoid overwhelming the 1.5B model's attention
    ...
    async def generate(self, state: State) -> Dict[str, Any]:
        ...
        try:
            ...
            error_msg = str(e)
            if "503" in error_msg or "Loading model" in error_msg:
                friendly_error = "120B大模型正在初始化加载中（约60GB数据需从硬盘读取至显存），请稍候约1-3分钟。服务完全就位后将自动恢复响应。"
                logger.warning(f"Model still loading: {friendly_error}")
                # We can yield a custom error message to the frontend through the stream_callback
                await self.stream_callback({"type": "token", "data": f"\n\n> [!IMPORTANT]\n> {friendly_error}\n\n"})
                raise e # Still raise to break the graph node correctly
            raise e
    ...
    async def query(self, query_text: str, chat_id: str, image_data: str = None) -> AsyncIterator[Dict[str, Any]]:
        ...
        try:
            ...
            # Heuristic Router: Pre-emptive RAG for Chinese queries
            # The 1.5B model struggles to call tools for non-English queries. 
            # We force-execute the search here and inject the context directly.
            # Heuristic Router: Pre-emptive RAG for Chinese queries
            # The 1.5B model struggles to call tools for non-English queries. 
            # We force-execute the search here and inject the context directly.
            final_query_text = query_text
            
            # STRICT MODE: Only pre-emptively search if sources are actually selected.
            current_config = self.config_manager.read_config()
            # ChatConfig is a Pydantic model, access attributes directly
            has_selected_sources = bool(current_config.selected_sources) if hasattr(current_config, "selected_sources") else False
            
            if any(u'\u4e00' <= c <= u'\u9fff' for c in query_text) and has_selected_sources:
                logger.info(f"Detected Chinese query with selected sources, executing PRE-EMPTIVE RAG for chat {chat_id}")
                try:
                    # Manually execute search (this includes the fallback logic implemented inside)
                    search_result = self.search_knowledge_base(query_text)
                    
                    if "No relevant documents" not in search_result:
                        logger.info("Pre-emptive RAG successful, injecting context into User Message.")
                        # STUFFING CONTEXT INTO USER MESSAGE (Stronger attention than SystemMessage)
                        final_query_text = f"{query_text}\n\n[SYSTEM INJECTED CONTEXT from Knowledge Base]:\n{search_result}\n\n[INSTRUCTION]: Answer based ONLY on the above context."
                except Exception as e:
                    logger.error(f"Pre-emptive RAG failed: {e}")

            messages_to_process.append(HumanMessage(content=final_query_text))
