...
export default function Sidebar({
  ...
  const [viewingSource, setViewingSource] = useState<{ name: string, content: string } | null>(null);
  ...
  const fetchChatMetadata = useCallback(async (chatId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        setChatMetadata((prev: Record<string, ChatMetadata>) => ({
          ...prev,
          [chatId]: metadata
        }));
      }
  ...
  const handleSourceToggle = async (source: string) => {
    ...
    if (selectedSources.includes(source)) {
      ...
      newSelectedSources = selectedSources.filter((s: string) => s !== source);
    } else {
  ...
  const handleViewSource = async (source: string) => {
    try {
      const response = await fetch(`/api/source/${source}/content`);
      if (response.ok) {
        const data = await response.json();
        setViewingSource({ name: source, content: data.content });
      } else {
  ...
  const handleDeleteSource = async (source: string) => {
    if (!window.confirm(`Are you sure you want to delete "${source}"? This will remove it from the vector store and disk.`)) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        await fetchSources();
        // If it was selected, it will be automatically removed from selectedSources state by the refresh logic if we fetch again, 
                      ...
                      availableSources.map(source => (
                        <div key={source} className={styles.sourceItem}>
                          <div className={styles.sourceMain}>
                            ...
                            />
                            <label htmlFor={`source-${source}`} title={source}>{source}</label>
                          </div>
        ...
        <div className={styles.modalOverlay} onClick={() => setViewingSource(null)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h2 className={styles.modalTitle}>{viewingSource.name}</h2>
              <button className={styles.modalClose} onClick={() => setViewingSource(null)}>×</button>
            </div>
            <div className={styles.modalContent}>
              {viewingSource.content || "No text content found."}
            </div>
          </div>
        </div>
      )}
